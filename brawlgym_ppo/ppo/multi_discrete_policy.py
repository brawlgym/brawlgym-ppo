...
Description:
    An implementation of a feed-forward neural network which parametrizes one discrete distribution per action for a
...
import torch
import numpy as np
from brawlgym_ppo.util import torch_functions


class MultiDiscreteFF(nn.Module):
    def __init__(self, input_shape, bins, layer_sizes, device):
        super().__init__()
        self.device = device
        bins = list(bins)
    ...
    def get_action(self, obs, deterministic=False):
        ...
        if deterministic:
            ...
            for split in self.splits:
                ...
                start += split
            action = torch.stack(action, dim=-1).detach().cpu().numpy()
            return action, 0
